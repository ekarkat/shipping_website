...
class Parcel(BaseModel, Base):
    ...
    parcel_history = Column(String(1024), nullable=True)
    parcel_cost = Column(String(60), nullable=True)
    parcel_type = Column(String(60), nullable=True)
    parcel_user_id = Column(String(128), ForeignKey("users.id"), nullable=False)
    parcel_agent_id = Column(String(128), ForeignKey("agents.id"), nullable=True)
